...
my $dir_work = $ARGV[2];
my $f_refer = "$dir_work/potentials/DDP/DDP_refer_CB.3.25.0.5.3";
####################################
...
for my $k1 (sort {$a<=>$b} keys %coords){
    ...
    for my $k2 (sort {$a<=>$b} keys %coords){
        ...
        if($dist <= $range2 && $dist >= $range1 && $kij >= $separation){ # exist potential score
          my $asp = $penalty;
          $check = 1;
          my $class = get_class($range1, $range2, $bin, $dist);
          if($class == 0){ next; }
