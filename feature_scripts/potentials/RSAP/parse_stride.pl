...
sub parse_stride{
  ...
  while(my $line = <IN>){
    ...
    if(substr($line, 0, 4) eq "ASG " ){
        ...
        my $residue_code = $amino{$residue_name};
        if($residue_code ~~ @aacode){
          my $racc = $acc/$Maxacc{$residue_code};
          push(@{$stride->{$pdb_residue_number}}, $residue_name);
          push(@{$stride->{$pdb_residue_number}}, $ss_code_3);
          push(@{$stride->{$pdb_residue_number}}, $racc);
        }
    }
  }
  close IN;
